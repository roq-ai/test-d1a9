...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Service Request Created.create': {
    roles: ['building-owner', 'property-manager', 'tenant', 'service-provider'],
    key: 'service_request',
    tenantPath: [],
    userPath: [],
  },
  'Maintenance Task Scheduled.create': {
    roles: ['building-owner', 'tenant', 'service-provider'],
    key: 'maintenance_task',
    tenantPath: [],
    userPath: [],
  },
  'Lease Agreement Signed.create': {
    roles: ['building-owner', 'property-manager'],
    key: 'lease_agreement',
    tenantPath: [],
    userPath: [],
  },
...
const ownerRoles: string[] = ['building-owner'];
const customerRoles: string[] = ['guest'];
const tenantRoles: string[] = ['building-owner', 'property-manager', 'tenant', 'service-provider', 'guest'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.landlord.findFirst({
