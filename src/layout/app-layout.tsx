...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['building-owner', 'property-manager', 'tenant', 'service-provider', 'guest'];
  ...
  const { data, error, isLoading } = useSWR<LandlordInterface[]>(
    ...
    () => getLandlords({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    { name: 'Properties', path: '/properties', entity: 'property', service: AccessServiceEnum.PROJECT, icon: FiMapPin },
    {
    ...
    { name: 'Tenants', path: '/tenants', entity: 'tenant', service: AccessServiceEnum.PROJECT, icon: FiUsers },
